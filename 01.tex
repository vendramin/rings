...
Most students will have some familiarity with
real polynomials from their school days. 

\begin{example}
\index{Polynomial!degree of}
\index{Ring!of polynomials}
...
\begin{enumerate}
    ...
    \item $(-x)y=x(-y)=-xy$ for all $x,y\in R$.
    \item If $1=0$, then $|R|=1$. 
\end{enumerate}
\end{exercise}

\begin{example}
\index{Ring!of quaternions}
...
\begin{example}
\index{Ring!of integers modulo n@of integers modulo $n$}
    Let $n\geq2$.  The abelian group $\Z/n=\{0,1,\dots,n-1\}$ of integers
    modulo $n$ is a ring with the usual multiplication modulo $n$. 
\end{example}

\begin{example}
  \index{Ring!of matrices}
    Let $n\geq1$. 
    ...
    \]
    Note that $M_n(\R)$ is commutative if and only if $n=1$. 
\end{example}
...
\label{exa:direct_rings}
    ...
    the identity element of $R\times S$ is $(1_R,1_S)$. The ring 
...
\index{Subring}
    ...
    $(S,+)$ is a subgroup of $(R,+)$ with $1\in S$ and 
...
\begin{example}
    ...
    This is known as the ring of \emph{Gaussian integers}.  
\end{example}

\index{Square-free integer}
...
\begin{example}
  Let $N\ne1$ be a square-free integer. Then 
  \[
    \Z[\sqrt{N}]=\{a+b\sqrt{N}:a,b\in\Z\}
  \]
  is
	a subring of $\C$.  	
\end{example}

If $N\ne1$ is a square-free integer, then 
...
Why does the 
equality $a+b\sqrt{2}=c+d\sqrt{2}$ in 
$\Q[\sqrt{2}]$
imply $a=c$ and $b=d$? 
...
\begin{exercise}
  ...
  \begin{enumerate}
    ...
    \item If $R_1\subseteq R_2\subseteq\cdots$ is a sequence of subrings of $R$, then 
      $\bigcup_{i\geq1}R_i$ is a subring of $R$. 
...
The set $\mathcal{U}(R)$ of units of a ring $R$ forms  
a group under multiplication. For example, $\mathcal{U}(\Z)=\{-1,1\}$ and 
...
\begin{exercise}
  \label{xca:units}
  Let $R$ be a commutative ring and $x_1,\dots,x_n\in R$.  Prove that if
  $x_1\cdots x_n$ is a unit, then each $x_j$ is a unit. 
\end{exercise}
...
The ring $H(\R)$ of real quaternions is a non-commutative division ring.
Find the inverse of an arbitrary non-zero element $a1+bi+cj+dk\in H(\R)$. 
...
\index{Ideal!right}
Similarly, one defines \emph{right ideals} replacing 
the condition $RI\subseteq I$ by 
the inclusion 
$IR\subseteq I$. 
...
\begin{exercise}
  ...
  Let $R$ be a ring. 
  \begin{enumerate}
    \item If $\{I_\alpha:\alpha\in\Lambda\}$ is a collection of ideals of $R$, then $\bigcap_{\alpha}I_\alpha$ is an ideal of $R$.  	
    \item If $I_1\subseteq I_2\subseteq\cdots$ is a sequence of ideals of $R$, then $\bigcup_{i\geq1}I_i$ is an ideal of $R$. 
  \end{enumerate}
\end{exercise}
...
\index{Ideal!generated by a set}
If $R$ is a ring and $X$ is a subset of $R$, one defines
the \emph{ideal generated} by $X$ as the smallest ideal of $R$ containing $X$, that is 
\[
...
\]
where the elements $x_1,\dots,x_m$ are not necessarily distinct and, by
convention, the empty sum is equal to zero. If $X=\{x_1,\dots,x_n\}$ is a
finite set, then we write $(X)=(x_1,\dots,x_n)$. 
...
\label{xca:ideals_matrices}
    Let $n\geq1$ and let $R$ be a commutative ring. Prove that every ideal of
    $M_n(R)$ is of the form $M_n(I)$ for some ideal $I$ of $R$. 
\end{exercise}
% I don't need commutativity here
...
The ring $\R[X]$ of real polynomials also has a division algorithm. Given the
polynomials $f(X)\in\R[X]$ and $g(X)\in\R[X]$ with $g(X)\ne 0$, there are unique 
...
Since the identity element $1$ of a ring $R$ 
is part of the structure, in 
the definition
...
\index{Ring!homomorphism!kernel}
\index{Ring!homomorphism!image}
...
\begin{example}
    Let $R$ be a non-zero ring and $R\times R$ be the direct product (see
    Example~\ref{exa:direct_rings}).  The map $R\to R\times R$, $r\mapsto
    (r,r)$ is an injective ring homomorphism. 

    For a ring $S$, the map 
    \[
    \pi_R\colon R\times S\to R,\quad (r,s)\mapsto r,
    \]
    is a surjective ring homomorphism with $\ker\pi_R=\{(0,s):s\in
    S\}=\{0\}\times S$. Note that $\ker\pi_R$ is not a subring of $R\times S$.
\end{example}

\begin{example}
  The map $\C\to M_2(\R)$, $a+bi\mapsto\begin{pmatrix}a&b\\-b&a\end{pmatrix}$,
  is an injective ring homomorphism. 	
\end{example}

\begin{example}
The map $\Z[i]\to\Z/5$, $a+bi\mapsto a+2b\bmod 5$, is a ring homomorphism 
with kernel $\{a+bi:a+2b\equiv 0\bmod 5\}$. 	
\end{example}

\begin{exercise}
\label{xca:Z6->Z15}
Prove that there is no ring homomorphism $\Z/6\to\Z/15$. 
\end{exercise}
...
$f$ to $\R$ is the identity, then there exists $x_0\in\R$ such that 
