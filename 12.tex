...
Since $R$ is a euclidean domain, $R$ is noetherian. Hence $R^k$ is a noetherian 
module, so 
$\ker\varphi$ is finitely generated. Let 
...
and $\{e_1,\dots,e_l\}$. 
\index{Matrix!of relations}

Multiplying a relations matrix 
on the left by an invertible matrix changes the generators of the relations module: 
...
Now multiplying a relations matrix on the right by an invertible matrix 
changes the original generating set of the module and the relations module: 
...
\begin{claim}
	...
	$\ker\psi$. Moreover, $AQ$ is the relations matrix with respect to
	$\{n_1,\dots,n_k\}$.  
\end{claim}
...
\]
In particular, 
each $m_i$ is a linear combination of $n_1,\dots,n_k$. Since 
$\{m_1,\dots,m_k\}$ generates $M$, so does $\{n_1,\dots,n_k\}$; equivalently, 
$\psi$ is surjective, as 
%$n_j=\psi(0,\dots,0,1,0,\dots,0)$ for all $j$.
%the elements 
\begin{align*}
...
\begin{proof}
Let $D=PAQ$. Then $D$ is a relations matrix for $M$ with respect to the
generating set $\{n_1,\dots,n_k\}$ determined by $Q^{-1}$. Let
\[
\psi\colon R^k\to M,\qquad
(r_1,\dots,r_k)\mapsto\sum_{i=1}^k r_i\cdot n_i.
\]
Then the rows of $D$ generate $\ker\psi$, and hence
\[
\ker\psi
=
(a_1)\oplus\cdots\oplus(a_r)\oplus\{0\}^{k-r}
\subseteq R^k.
\]
Therefore
\[
M\simeq R^k/\ker\psi
\simeq
R/(a_1)\oplus\cdots\oplus R/(a_r)\oplus R^{k-r},
\]
by Exercise~\ref{xca:easy_submodules}.
\end{proof}
...
%the \emph{Smith normal form} of the matrix $A$. 
\index{Smith normal form}
\index{Invariant factors}
The diagonal matrix $PAQ$ is a \emph{Smith normal form} of $A$.
The decomposition in the previous proposition is the corresponding
\emph{invariant-factor decomposition} of $M$. 
...
corresponds to multiplying $A$ on the left by a permutation matrix. 
The second
operation corresponds to multiplying $A$ by $I+\lambda E_{ij}$ on the left, 
where 
\[
...
\end{pmatrix}.
\]
Similarly, column operations correspond to multiplying the matrix $A$ on the right 
either by a permutation matrix or a matrix of the form $I+\lambda E_{ij}$. 

\begin{theorem}[Smith normal form]
\index{Smith normal form}
Let $R$ be a euclidean domain. If $A\in R^{l\times k}$,  
there exist invertible matrices $P\in R^{l\times l}$ and $Q\in R^{k\times k}$ such that 
\[
PAQ=\begin{pmatrix}
a_1 & 0 & \cdots & 0 & 0 & \cdots & 0\\
0 & a_2 & \cdots & 0 & 0 & \cdots & 0\\
...
\begin{proof}[Sketch of the proof]
	We only prove the existence. Let $(R,\varphi)$ be a euclidean domain.  
...
\[
\begin{pmatrix}
	\frac{b_{22}}{b_{11}} & \cdots & \frac{b_{2k}}{b_{11}}\\
	\vdots & &\vdots \\
	\frac{b_{l2}}{b_{11}} & \cdots & \frac{b_{lk}}{b_{11}}
\end{pmatrix}
\]
and repeat the method until we cannot continue. Note that row and column
operations on the submatrix above correspond to the same operations on the
submatrix of $B$; so if the former is reduced to the diagonal matrix with
entries $c_2,c_3,\dots,c_r$ with $c_2\mid\cdots\mid c_r$, the original matrix
is reduced to the diagonal matrix with entries
$b_{11},b_{11}c_2,\dots,b_{11}c_r$, which has the required divisibility.

Let us show how to get the matrix $B$. If $A=0$, there is nothing to prove. 
Assume that $A\ne0$. By applying row and column operations we may assume that 
a non-zero entry of $A$ having minimal Euclidean value 
appears 
in position
$(1,1)$.  
...
$(1,j)$.  
In either case, move the non-zero remainder to position $(1,1)$
by a row or column interchange and repeat. The Euclidean value of
the entry in position $(1,1)$ strictly decreases. 

If every $a_{i1}$ with $i>1$ is divisible by $a_{11}$, say $a_{i1}=a_{11}\lambda_i$, then
apply $R_i\leftarrow \lambda_i R_1-R_i$. Similarly, if 
every $a_{1j}$ with $j>1$ is divisible by $a_{11}$, say 
$a_{1j}=a_{11}\mu_j$ for some $\mu_j$, then
...
Since the Euclidean values are non-negative integers, this process
terminates. We eventually obtain a non-zero entry $a_{11}$ that
divides every entry of the matrix. We then clear the rest of the
first row and first column and apply induction to the remaining
submatrix.
\end{proof}

We refer to Artin's book \cite[\S14]{MR1129886} for 
a detailed exposition of the Smith normal form 
when the base ring $R$ is $\Z$ or
$K[X]$ for any field $K$. For an application 
of the Smith normal form to explicit calculations related to 
...
Let us compute the Smith normal form of $A$.
...
\end{pmatrix}.
\]
To obtain zeros in positions $(1,2)$, $(1,3)$, $(2,1)$ and $(3,1)$ we 
...
\end{pmatrix}.
\]
We perform the same procedure to the submatrix
$\begin{pmatrix}10&4\\13&3\end{pmatrix}$.  We want the smallest entry in the
upper-left position of this submatrix, which is position $(2,2)$ in the
original matrix.  For that purpose, apply $R_1\leftrightarrow R_2$ and
...
%zero columns. 
Suppose that an $m\times n$ relations matrix has Smith normal form
with non-zero diagonal entries $a_1,\dots,a_r$. Then
\[
M\simeq
R/(a_1)\oplus\cdots\oplus R/(a_r)\oplus R^{n-r}.
\]
Thus every zero column contributes a free factor $R$. If $m>n$,
the additional rows in the Smith normal form are zero rows and
correspond to redundant relations.
...
\]	
We claim that 
$M\simeq\Z\oplus\Z/4\oplus\Z/12$.
Apply 
...
\end{pmatrix},
\]
which corresponds to generators $\{m_1+2m_2+m_3,m_2,m_3\}$ 
and relations 
\[ 
12m_2=0,\quad 
4(m_1+2m_2+m_3)=0.
...
the set of generators $\{m_1+2m_2+m_3,m_2,m_3\}$ and relations 
\[ 
4(m_1+2m_2+m_3)=0,\quad 
12m_2=0.
\]
There is no relation on the third generator $m_3$ in this new
generating set, so it generates a free summand isomorphic to $\Z$. 
The row operation used corresponds to left multiplication by the 
permutation matrix 
$\begin{pmatrix}0&1\\1&0\end{pmatrix}$. 
Thus $M\simeq\Z\oplus\Z/4\oplus\Z/12$. We also obtained that
\begin{align*}
&P=
\begin{pmatrix}
    0&1\\
...
Note that the matrices $P$ and $Q$ are not uniquely determined. Here is the
Magma code that solves the previous example.
\begin{lstlisting}
...
\begin{example}
Let $M$ be the free abelian group with basis $\{m_1,\dots,m_4\}$ and let $K$
...
\end{pmatrix}. 
\]
Note that $4\nmid 22$. Apply $R_2\leftarrow R_2+R_3$, $C_3\leftarrow C_3-5C_2$,
$C_2\leftrightarrow C_3$, $R_3\leftarrow R_3-11R_2$ and $C_3\leftarrow C_3-2C_2$ to
get 
\[
\begin{pmatrix}
	2 & 0 & 0 & 0\\
...
\end{pmatrix}.
\]
The group $M/K$ has generators $\{n_1,n_2,n_3,n_4\}$ and relations 
...
\begin{example}
	...
	Smith normal form of $A$. For example, computer calculations show that
	\[
	P=\begin{pmatrix}
		0 & 1\\
		-1 & 1
	...
	\end{pmatrix}.
	\]
  Note that the matrix $S$ is unique up to units (up to signs over $\Z$), but
  not the matrices $P$ and $Q$. To solve $AX=b$ we proceed as follows. Let
  $Y=Q^{-1}X$. Then $AX=b$ implies that 
	...
	$Y=Q^{-1}X=\begin{pmatrix}
		...
		y+z\\
		y
	\end{pmatrix}$ and hence $SY=\begin{pmatrix}5\\6\end{pmatrix}$ turns into 
	\[
		\begin{pmatrix}
			x+2z\\
			y+z
...
\index{Matrix!unimodular}
    Let $G$ be a free abelian group of rank $n$ with basis $\{x_1,\dots,x_n\}$.
