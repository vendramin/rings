...
Since $R$ is an euclidean domain, $R$ is noetherian. Hence $R^k$ is a noetherian 
module, so 
...
\begin{proof}
Let $D=PAQ$. Then $D$ is a relation
matrix for $M$ with respect to the generating set
$\{n_1,\dots,n_k\}$ determined by $Q^{-1}$. Let
\[
...
every $a_{1j}$ with $j>1$ is divisible by $a_{11}$, say $a_{1j}=a_{11}\mu_j$, then
...
of the Smith's normal form to explicit calculations related to 
...
%zero columns. 
Suppose that an $m\times n$ relation matrix has Smith normal form
with non-zero diagonal entries $a_1,\dots,a_r$. Then
\[
M\simeq
\Z/(a_1)\oplus\cdots\oplus\Z/(a_r)\oplus\Z^{n-r}.
\]
Thus every zero column contributes a free factor $\Z$. If $m>n$,
...
$C_2\leftarrow C_3$, $R_3\leftarrow R_3-11R_2$ and $C_3\leftarrow C_3-2C_2$ to
get 
\[
\begin{pmatrix}
	2 & 0 & 0 & 0\\
