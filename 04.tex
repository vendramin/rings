\lecture\label{lec:MoreFactorization}
...
In the integers primes and irreducibles coincide. This happens 
...
and hence admits factorizations into irreducibles. However, not
...
\begin{proof}
	...
	\begin{claim}
  \label{claim:noetherian=>factorization}
...
The following picture shows the relationships between
different classes of commutative domains:
...
\begin{tikzpicture}[
  ...
  lab/.style   = {font=\scriptsize, inner sep=1.5pt, fill=white,
    ...
    \draw[imp] (N)  -- node[lab, above] {Page~\pageref{claim:noetherian=>factorization}} (F);
\end{tikzpicture}
...
$\Z[\frac{1+\sqrt{-19}}{2}]$ is principal and not euclidean, and
$\Z[X]$ is a unique factorization domain that is not principal.
