...
\begin{example}
    ...
    \]
    be the symmetric group in three letters. Every element of $\C[\Sym_3]$
    is of the form 
    \[
...
\index{Algebra!isomorphism}
An \emph{algebra homomorphism} is a ring homomorphism that is $K$-linear. An
\emph{algebra isomorphism} is a bijective algebra homomorphism; we use the
symbol $\simeq$ for isomorphisms, as we did for rings.

\begin{example}
	Let $G=\langle g:g^3=1\rangle=\{1,g,g^2\}\simeq C_3$ be the cyclic group of order three. 
	If $\alpha=a_11+a_2g+a_3g^2$ and $\beta=b_11+b_2g+b_3g^2$, then
	\[
...
The group algebra has the following property, which is left as an exercise.
Let $A$ be a $K$-algebra and $G$ be a finite group. If $f\colon
G\to\mathcal{U}(A)$ is a group homomorphism, then there exists a unique
$K$-algebra homomorphism $\varphi\colon K[G]\to A$ such that $\varphi|_G=f$. 
...
\begin{example}
	...
	\]
  Let 	
  $\omega$ be a primitive cubic root of unity. Let 
  $A=\C\times\C\times M_2(\C)$ and  
	\[
	R=\begin{pmatrix}
		\omega&0\\
		0&\omega^2	
  ...
  $\D_3\to\mathcal{U}(A)$ such that 
	$r\mapsto (1,1,R)$ and $s\mapsto (1,-1,S)$. This homomorphism
  extends uniquely to an algebra homomorphism $\C[\D_3]\to A$.  
  A direct calculation shows that the images of
  \[
  1,r,r^2,s,rs,r^2s
  \]
  are linearly independent. Since both algebras have dimension six,
  $\C[\D_3]\simeq A$. 
%	Let $\D_3=\langle r,s:r^3=s^2=1,\,srs^{-1}=r^{-1}\rangle$ be the dihedral
%	group of six elements. We claim that 
%	\[
...
The \emph{degree} of the representation $\rho\colon G\to\GL(V)$ is the dimension of $V$. 
...
\begin{example}
	...
	different for all $n$. It is enough to show that if 
...
\begin{definition}
    ...
    if there exists a bijective invariant linear 
    map $T\colon V\to W$. 
\end{definition}
...
\begin{example}
    ...
    are equivalent, as $\rho_mT=T\psi_m$ for all $m\in G$ if $T=\begin{pmatrix}
        ...
        1&1
    \end{pmatrix}$.
    Thus $T$ is an invariant isomorphism from $\psi$ to $\rho$;
    equivalently, $T^{-1}$ is an invariant isomorphism from $\rho$
    to $\psi$.
\end{example}
...
\]
be the coordinate isomorphism determined by 
the basis $B$. For each $g\in G$, 
...
\index{Representation!restriction}
Let $\rho\colon G\to\GL(V)$ be a representation. Then $\{0\}$ and $V$ are always   
invariant subspaces of~$V$. Moreover, if $W\subseteq V$ is non-zero and 
invariant, then
...
\begin{definition}
    ...
    A representation $\rho\colon G\to\GL(V)$ is \emph{irreducible} if
...
In the following example, it is crucial to use a real representation.
...
\begin{example}
    ...
    \end{pmatrix}\in\R^3:x+y+z=0\right\}
    \]
    is an invariant subspace of $\R^3$. We claim that the restricted representation 
    \[
      \rho|_W\colon G\to\GL(W)
    \]
...
\begin{exercise}
\label{xca:common_eigenvector}
    Let $\rho\colon G\to\GL(V)$ be a degree-two representation. Prove that
    $\rho$ is irreducible if and only if there is no common eigenvector for the
    $\rho_g$, $g\in G$.
\end{exercise}
...
\index{Representation!direct sum of}
...
\]
where $(\rho\oplus\psi)_g\colon V\oplus W\to V\oplus W$, $(v,w)\mapsto
(\rho_g(v),\psi_g(w))$. Note that both $V\oplus\{0\}$ and $\{0\}\oplus W$ are
...
\begin{definition}
    ...
    $\rho\simeq\rho_1\oplus\cdots\oplus \rho_n$ for some irreducible
    representations $\rho_1,\dots,\rho_n$ of $G$.
Equivalently, 
there exist invariant subspaces $V_1,\dots,V_n$ such that
\[
V=V_1\oplus\cdots\oplus V_n
\]
and every restricted representation
\[
\rho|_{V_i}\colon G\to\GL(V_i)
\]
is irreducible.
\end{definition}

Note that if $\rho\colon G\to\GL(V)$ is completely reducible and 
$\rho\simeq\rho_1\oplus\cdots\oplus \rho_n$ for some irreducible representations 
...
$\{g:g\in G\}$. The \emph{Representation}
