\section{Modules}
\lecture\label{lec:Modules}

The rest of the course will be devoted to studying modules over rings. 
We start with the main definitions and basic examples.
...
Our definition is that of a left module. Similarly, one defines right modules.
We will always consider left modules (over $R$) so we will refer to them 
simply as 
$R$-modules (or just modules).
...
\begin{example}
    ...
    \[
      \left(\sum_{g\in G}\lambda_g g\right)\cdot v
      =\sum_{g\in G}\lambda_g\rho_g(v).
    \]

    Conversely, if $V$ is a non-zero finite-dimensional 
    $\C[G]$-module, then 
    the map
    \[
...
\begin{example}
If $R$ is a ring, then $M_{m,n}(R)$ is a module (over $R$) with addition
...
$(s,m)\mapsto sm$, then 
$M$ is also a module (over $R$) with $r\cdot m=f(r)m$ for all $r\in R$ and
$m\in M$. In fact, 
\begin{align*}
&1\cdot m=f(1)m=1m=m,\\
%&(r_1r_2)\cdot m=r_1\cdot (r_2\cdot m)=f(r_1)(r_2\cdot m)=f(r_1)(f(r_2)m)=(f(r_1)f(r_2))m=f(r_1r_2)m
&r_1\cdot (r_2\cdot m)=f(r_1)(r_2\cdot m)=f(r_1)(f(r_2)m)=(f(r_1)f(r_2))m=f(r_1r_2)m=(r_1r_2)\cdot m
\end{align*}
...
\begin{example}
\label{exa:RX-module}
Let $V$ be a finite-dimensional real vector space and $T\colon V\to V$ be a
linear map.  Let $R=\R[X]$. Then $M=V$ with 
\[
...
\begin{example}
  \index{Module!direct product of}
...
\begin{example}
  \index{Module!direct sum of}
If $\{M_i:i\in I\}$ is a family of $R$-modules, then   	
\[
\bigoplus_{i\in I}M_i=\{(m_i)_{i\in I}:m_i\in M_i\text{ for all $i\in I$ and $m_i=0$ except for finitely many $i\in I$}\}
\]
is an $R$-module with 
$x\cdot (m_i)_{i\in I}=(x\cdot m_i)_{i\in I}$. 
...
It is an exercise to show that if $M$ is a module, then $0\cdot m=0$ and
$-m=(-1)\cdot m$ for all $m\in M$ and $x\cdot 0=0$ for all $x\in R$. 
...
\begin{definition}
  \index{Submodule}
	Let $M$ be an $R$-module. A subset $N$ of $M$ is a \emph{submodule} of $M$ if 
...
%\left(\sum_{i=0}^na_iX^i\right)\cdot v=\sum_{i=0}^na_iT^i(v).
%\]
In the module of Example~\ref{exa:RX-module}, 
a submodule is a subspace $W$ 
of $V$ such that $T(W)\subseteq W$. 
\end{example}

Clearly, a non-empty subset $N$ of $M$ is a submodule if and only 
...
\index{Module!homomorphism!kernel}
\index{Module!monomorphism}
\index{Module!epimorphism}
\index{Module!isomorphism}
...
\item For every module $V$ and every $g\in\Hom_R(V,M)$, $fg=0\implies g=0$.
\end{enumerate}
\end{exercise}
...
\index{Direct summand}
\index{Complement of a submodule}
If $N$ and $N_1$ are submodules of $M$, we say that $M$ is the \emph{direct
sum} of $N$ and $N_1$ if $M=N+N_1$ and $N\cap N_1=\{0\}$. In this case, we
write $M=N\oplus N_1$. Note that if $M=N\oplus N_1$, then each $m\in M$ can be
written uniquely as $m=n+n_1$ for some $n\in N$ and $n_1\in N_1$.  Such a
decomposition exists because $M=N+N_1$. If $m\in M$ can be written as
$m=n+n_1=n'+n_1'$ for some $n,n'\in N$ and $n_1,n_1'\in N_1$, then
$-n'+n=n_1'-n_1\in N\cap N_1=\{0\}$ and hence $n=n'$ and $n_1=n_1'$. If
$M=N\oplus N_1$, the submodule $N$ (resp. $N_1$) is a \emph{direct summand} of
$M$ and the submodule $N_1$ (resp. $N$) is a \emph{complement} of $N$ (resp. $N_1$) in $M$.   	
...
%\end{example}
\begin{example}
Let $M=\Z$ as a module over $\Z$ and let $m\in\Z_{\geq0}$. Then
$m\Z$ is a direct summand of $M$ if and only if $m\in\{0,1\}$.
Indeed, suppose that
\[
\Z=m\Z\oplus n\Z
\]
for some $n\in\Z_{\geq0}$. Since
$m\Z\cap n\Z=\{0\}$, it follows that $mn=0$. If $m\ne0$, then
$n=0$, and hence $m\Z=\Z$, so $m=1$. Conversely, both $\{0\}$
and $\Z$ are direct summands of $\Z$.
\end{example}
...
If $V_1,\dots,V_n$ are submodules of $M$, we say that $M=V_1\oplus\cdots\oplus
V_n$ if every $m\in M$ can be written uniquely as $m=v_1+\cdots+v_n$ for some
$v_1\in V_1,\dots,v_n\in V_n$. 

\begin{exercise}
Prove that $M=V_1\oplus\cdots\oplus V_n$ if and only if 
$M=V_1+\cdots+V_n$ and 
\[
V_i\bigcap\left(\sum_{j\ne i}V_j\right)=\{0\}
\]	
for all $i\in\{1,\dots,n\}$.
\end{exercise}

If $\{N_i:i\in I\}$ is a family of submodules of a module $M$, then the intersection  
$\bigcap_{i\in I}N_i$ is also a submodule of $M$.
...
\begin{exercise}
\label{xca:Hom}
Prove that $\Hom_R(M,N)$ is a module over $Z(R)$ with the following action: 
If $r\in Z(R)$ and
$f\in\Hom_R(M,N)$, then $r\cdot f\colon M\to N$, $m\mapsto f(r\cdot m)$. 
\end{exercise}

\index{Module!quotient}
...
\begin{enumerate}
\item $N=\ker \pi$.
\item If $T$ is an $R$-module and $f\colon M\to T$ is a module 
  homomorphism such that $N\subseteq \ker f$, then there exists a 
...
\index{Isomorphism theorems!for modules}
...
\begin{example}
If $K$ is a field and $V$ is a finite-dimensional 
module over $K$, then $V$ is, by definition, a vector space over $K$. 
If $S$ and $T$ are subspaces of $V$, then they are submodules of $V$. 
By the second isomorphism theorem, $(S+T)/S\simeq T/(S\cap T)$ 
...
\label{xca:mod_iso_max}
    Let $R$ be a ring and $M_1$ and $M_2$ be maximal ideals of $R$. Prove
    that $R/M_1\simeq R/M_2$ as modules over $R$ if and only if there exists
